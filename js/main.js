...
(function () {
  ...
  if (!body || body.getAttribute("data-lazyload") === "false") return;

  var lazyImages = Array.prototype.slice.call(document.querySelectorAll("img.db8gr-lazy-media[data-src]"));
  if (!lazyImages.length) return;

  var reducedMotion = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  var rootMargin = body.getAttribute("data-lazyload-root-margin") || "240px 0px";

  function markLoaded(img) {
    img.classList.add("is-lazy-loaded");
    img.classList.remove("is-lazy-loading");
  }

  function markError(img) {
    img.classList.add("is-lazy-error");
    img.classList.remove("is-lazy-loading");
  }

  function bindState(img) {
    if (img.__db8grLazyBound) return;
    img.__db8grLazyBound = true;
  ...
  function activate(img) {
    if (!img || img.__db8grLazyActivated) return;
    img.__db8grLazyActivated = true;
    img.classList.add("is-lazy-loading");
    bindState(img);
